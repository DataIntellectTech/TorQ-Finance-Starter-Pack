...
get_quote:{tab:raze{[sym]
   ...
   tab:check_dupq/[0#tab;tab];                 / Check for duplicate data
...
check_dupq:{[x;y]
  :x upsert $[(nullq~qcols#y)or(qcols#y)~qcols#.iex.lvcq y`sym;
   ();
   [`.iex.lvcq upsert y;y]
   ]; 
 };

qcols:`bid`ask`bsize`asize`mode`ex;
nullq:qcols!(0f;0f;0;0;" ";" ");
