...
getTableSchema:{													/ function to retrieve empty tables from schema file 
  ...
  /.dqe.schemaTables:{.schema[x]:value each x}.Q.pt;
  {(set')[.dqe.schemaTables:`$".schema.",/:string x;value each x]}.Q.pt;						/ rename each schema table, save names under schemaTables
 };

checkLogger:{[checkname;checkresult]											/ function to add a log wrapper to each check
  $[result:checkresult;													/ conditional to log to output or error logs depending on check result
...
checkColumnNames:{checkLogger["columnNames";(cols each .Q.pt)~`date,/:cols each .dqe.schemaTables]};			/ function to check if on-disk table column names match those of the schema
